...
AVLTree::AVLTree(int id, int score)
{
    insert(id, score);
}
    ...
    if (grandparent != nullptr) {
        if (node->parent == grandparent->left.get())
        {
            ...
            if (is_left)
            {
                ...
                if (parent->left != nullptr) parent->left->parent = parent.get();
            ...
            else
            {
                ...
                if (parent->right != nullptr) parent->right->parent = parent.get();
                parent->height = get_subtree_height(parent.get());
        ...
        else
        {
            ...
            if (is_left)
            {
                ...
                if (grandparent->right != nullptr) grandparent->right->parent = grandparent;
                parent->left = std::move(node->right);
                if (parent->left != nullptr) parent->left->parent = parent.get();
                parent->height = get_subtree_height(parent.get());
                node->right = std::move(parent);
